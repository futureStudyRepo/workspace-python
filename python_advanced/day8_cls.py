# python 은 OOP 객체지향프로그래밍을 지원함. 
# class는 객체를 만드는 설계도 
# 객체는 class를 인스턴스화 해서 생성함. 

class CoffeeFranchise:
    # init 생성자라고 하며 class를 인스턴스화 할때 
    # 필요한 인자값이 있다면 정의 해야함(없다면 안해도됨.)
    ...
    def make_ame(self): # class에 종속되어 있는 함수를 메서드라 함.
        if self.beans:  
            print(f"{self.branch_name} 에서는 {self.beans} 으로 커피를 만들어요.")
        else:
            print(f"{self.branch_name} 에서는 신선한 커피를 제공합니다. ")
future = CoffeeFranchise("미래융합점", "미래 블렌드", "미래 라때")
